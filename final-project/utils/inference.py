...
def infer_stationary_dirichlet_categorical_edward(x_data, x, pi_0, pi_T, n_states, chain_len, batch_size, n_samples=10, n_epoch=5, lr=0.005):
...
def infer_stationary_dirichlet_categorical_tfp(x_data, model, pi_0, pi_T, n_states, chain_len, batch_size, n_samples=10, n_epoch=1, lr=0.005):
  """ runs variational inference given mc model with priors """
...
def infer_non_stationary_dirichlet_categorical(x_data, x, pi_0, pi_T_list, n_states, chain_len, batch_size, n_samples=10, n_epoch=5, lr=0.005):
	...
	def function(x_data, x, pi_0, pi_T_list, n_states, chain_len, batch_size, n_samples=10, n_epoch=5, lr=0.005, **kwargs):
		sess = tf.Session()
		data = generator(x_data, batch_size)
...
def infer_stationary_dirichlet_multinomial(x_data, pi_list, counts, total_counts_per_month, n_states, chain_len, n_samples=10):
	""" runs variational inference given mc model with priors, conditioning on position in the chain """

	def function(x_data, pi_list, counts, total_counts_per_month, n_states, chain_len, n_samples=10, **kwargs):
		sess = tf.Session()
		...
		with sess.as_default():
			...
			save_path = saver.save(sess, join(cache_path, 'experiment5.ckpt'))
	...
	args = [x_data, pi_list, counts, total_counts_per_month, n_states, chain_len, n_samples]
	kwargs = {
		'format': 'table',
		'ed_model': {
