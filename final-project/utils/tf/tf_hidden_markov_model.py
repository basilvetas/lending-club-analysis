...
# as util this submodule does exist but doesn't have what we need...
# so add it ourselves:
...
# Trying this, from the Issue where they decided to implement
# broadcast_to: (https://github.com/tensorflow/tensorflow/issues/14509)
...
class distribution_HiddenMarkovModel(distribution.Distribution):
    ...
    The `HiddenMarkovModel` distribution implements a (batch of)
    hidden Markov models where the initial states, transition
    probabilities and observed states are all given by user-provided
    distributions. This model assumes that the transition matrices
    are fixed over time.

    In this model, there is a sequence of integer-valued hidden states:
    `z[0], z[1], ..., z[num_steps - 1]` and a sequence of observed
    states: `x[0], ..., x[num_steps - 1]`. The distribution of `z[0]`
    is given by `initial_distribution`. The conditional probability of
    `z[i  +  1]` given `z[i]` is described by the batch of distributions
    in `transition_distribution`. For a batch of hidden Markov models,
    the coordinates before the rightmost one of the `transition_distribution`
    batch correspond to indices into the hidden Markov model batch. The
    rightmost coordinate of the batch is used to select which distribution
    `z[i + 1]` is drawn from.  The distributions corresponding to the
    probability of `z[i + 1]` conditional on `z[i] == k` is given by the
    elements of the batch whose rightmost coordinate is `k`.
    Similarly, the conditional distribution of `z[i]` given `x[i]` is given
    by the batch of `observation_distribution`.
    ...
    def __init__(self,
        ...
        dimension of `observation_distribution`. transition_distribution:
        ...
        ValueError: if `initial_distribution` does not have scalar
        `event_shape`.
        ValueError: if `transition_distribution` does not have scalar
         `event_shape.`
        ValueError: if `transition_distribution` and
        `observation_distribution`
        ...
        with tf.name_scope(name=name, values=(
            ...
            if num_steps < 1:
                raise ValueError(f'num_steps ({num_steps}) must be at least 1')
            ...
            if (initial_distribution.event_shape is not None
                ...
                self._runtime_assertions += [tf.assert_equal(
            ...
            if (transition_distribution.event_shape is not None
                ...
                self._runtime_assertions += [tf.assert_equal(
                    ...
                    0, message="`transition_distribution` must have scalar"
                    "`event_dim`s")]
            ...
            if (transition_distribution.batch_shape is not None
                ...
                self._runtime_assertions += [tf.assert_greater(
            ...
            if (tf.contrib.framework.is_tensor(self._num_states) or
                ...
                raise ValueError(
                    "`transition_distribution` and "
                    "`observation_distribution` must agree on "
                    "last dimension of batch size")

            self._log_init = _extract_log_probs(
                self._num_states,
                initial_distribution)
            self._log_trans = _extract_log_probs(
                self._num_states,
                transition_distribution)
    ...
    def _sample_n(self, n, seed=None):
        with tf.control_dependencies(self._runtime_assertions):
            ...
            init_repeat = (
                    ...
                    tf.reduce_prod(
                        self._initial_distribution.batch_shape_tensor()))
            init_state = self._initial_distribution.sample(
                n * init_repeat,
                seed=seed())
            ...
            def generate_step(state, _):
                ...
                return tf.reduce_sum(
                    old_states_one_hot * new_states,
                    axis=-1)

            if self._num_steps > 1:
                dummy_index = tf.zeros(
                    self._num_steps - 1,
                    dtype=tf.float32)
            ...
            # possible_observations :: steps n batch_size num_states
            # inner_shape
            ...
            # hidden_one_hot :: steps n batch_size num_states
            # "inner_shape"
    ...
    def _log_prob(self, value):
        with tf.control_dependencies(self._runtime_assertions):
            # The argument `value` is a tensor of sequences
            # of observations.
            # `observation_batch_shape` is the shape of that
            # tensor with the
            # sequence part removed.
            # `observation_batch_shape` is then broadcast to
            # the full batch shape
            # to give the `working_shape` that defines the shape
            # of the result.

            observation_batch_shape = tf.shape(
                value)[:-1 - self._underlying_event_rank]
            # value :: observation_batch_shape num_steps
            ...
            # `observation_event_shape` is the shape of each
            # sequence of observations emitted by the model.
            ...
            # Move index into sequence of observations to front
            # so we can apply tf.foldl
            working_obs = util.move_dimension(
                working_obs,
                -1 - r,
                0)[..., tf.newaxis]
...
def _log_vector_matrix(vs, ms):
    """
    Multiply tensor of vectors by matrices assuming
    values stored are logs.
    """
